...
def _set_window_icon() -> None:
    """Set the window/Dock icon from icon.png (generated by the macOS
    `make icon` target). A missing or unreadable icon is non-fatal but is
    reported on stderr so generation failures are not silent."""
    icon_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "icon.png")
    try:
        pygame.display.set_icon(pygame.image.load(icon_path))
    except (pygame.error, OSError) as exc:
        print(f"[minesweeper] could not load window icon: {exc}", file=sys.stderr)


def main() -> None:
    pygame.init()
    _set_window_icon()  # must precede set_mode so the window adopts the icon
